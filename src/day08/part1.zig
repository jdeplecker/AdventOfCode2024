...
fn put_antinodes(antinode_board: std.ArrayList([]u8), antenna_positions: std.ArrayList(Pos)) void {
    for (antenna_positions.items, 0..) |antenna_pos_left, i| {
        for (antenna_positions.items, 0..) |antenna_pos_right, j| {
            ...
            const anti_position1 = Pos{ .x = antenna_pos_left.x - (antenna_pos_right.x - antenna_pos_left.x), .y = antenna_pos_left.y - (antenna_pos_right.y - antenna_pos_left.y) };
            if (anti_position1.x >= 0 and anti_position1.y >= 0 and anti_position1.x < antinode_board.items.len and anti_position1.y < antinode_board.items.len) {
                antinode_board.items[anti_position1.uy()][anti_position1.ux()] = 'X';
            }
            const anti_position2 = Pos{ .x = antenna_pos_right.x + (antenna_pos_right.x - antenna_pos_left.x), .y = antenna_pos_right.y + (antenna_pos_right.y - antenna_pos_left.y) };
