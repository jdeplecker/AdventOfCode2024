    ...
    for (0..single_line_input.len) |input_letter_idx| {
        for (masks) |mask| {
            ...
            for (mask) |mask_tuple| {
                ...
                if (test_index >= single_line_input.len or single_line_input[test_index] != mask_tuple[0] or @max(test_index_mod, prev_index_mod) - @min(test_index_mod, prev_index_mod) > 1) { // don't wrap around
                    match = false;
                    break;
                }
