    ...
    while (lines.next()) |line| {
        ...
        if (try isSuccess(nums)) {
            ...
            for (0..nums.items.len) |i| {
                var small_nums = std.ArrayList(i32).init(allocator);
                defer small_nums.deinit();
                try small_nums.appendSlice(nums.items);
                _ = small_nums.orderedRemove(i);
                if (try isSuccess(small_nums)) {
                    success += 1;
                    break;
                }
            }
