...
pub fn main() !void {
    ...
    var result_path = try turn_penalty_dijikstra(&maze, start, end);
    defer result_path.deinit();

    print("Result: {}\n", .{result_path.items[result_path.items.len - 1].distance + 1});
}
...
fn turn_penalty_dijikstra(maze: *graph.Graph(i32), src: []const u8, dst: []const u8) !std.ArrayList(graph.Graph(i32).Element) {
    var result = std.StringHashMap(i32).init(maze.allocator);
    var path = std.ArrayList(graph.Graph(i32).Element).init(maze.allocator);

    if ((maze.vertices.?.contains(src) == false) or (maze.vertices.?.contains(dst) == false)) {
        return path;
    }

    const source: *graph.Graph(i32).Node = maze.vertices.?.get(src).?;

    var pq = std.PriorityQueue(graph.Graph(i32).Element, void, graph.Graph(i32).minCompare).init(maze.allocator, {});
    defer pq.deinit();

    var visited = std.StringHashMap(i32).init(maze.allocator);
    defer visited.deinit();

    var distances = std.StringHashMap(i32).init(maze.allocator);
    defer distances.deinit();

    var prev = std.StringHashMap(*graph.Graph(i32).Node).init(maze.allocator);
    defer prev.deinit();

    var prev_dir = std.StringHashMap(Dir).init(maze.allocator);
    defer prev_dir.deinit();

    // Initially, push all the nodes into the distances hashmap with a distance of infinity.

    var node_it = maze.vertices.?.keyIterator();
    ...
    while (pq.count() > 0) {
        ...
        if (!visited.contains(current.name)) {
            const currentPtr: *graph.Graph(i32).Node = maze.vertices.?.get(current.name).?;
            const neighbors: std.ArrayList(*graph.Graph(i32).Edge) = maze.graph.?.get(currentPtr).?;
